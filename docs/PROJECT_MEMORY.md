...
- **Summary:** Story compatibility now binds every frozen and current quest entity through fail-closed global identity indexes while retaining exact immutable fixture provenance and the approved commodity item-tag exception.
- **Evidence:** Independent review reproduced duplicate quest and chapter additions returning `[]` and one valid front insertion producing 41 false mismatches. Fix regressions first ended with `FAILED (failures=70)`. The first implementation run then ended with `FAILED (failures=1)` because reward items were misclassified as commodity task targets, and the hygiene edge cycle ended with `FAILED (failures=2)` for nil UUID and UNC path gaps. Deliberately removing the cross-corpus binding check made the saved-progress reuse regression end with `FAILED (failures=1)`. After restoring the binding and hygiene checks, all 26 focused compatibility tests passed. The unchanged fixture still matches exact Git object `7fcbc3a99fedcb8f6a62861ef86a2fd1e05fef25` at SHA-256 `b0e2fe06bb712e0f19f9fd3e94f5c4d75a570315c4d1956b6e95478b45df2d5c`.
- **Files or Commit:** `0568dae`, `tools/afterlight_quests/compatibility.py`, `tools/fixtures/quests/story-cohesion-baseline.json`, `tools/tests/test_afterlight_quests.py`, and the Task 2 fix-round commit
- **Impact:** Additive entities can appear at any position without rebinding saved progress, while missing IDs, duplicate IDs, cross-kind collisions, frozen-order mutations, undeclared payload changes, and fixture contamination fail with searchable paths.
