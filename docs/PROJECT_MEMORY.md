...
- **Impact:** Slug links resolve to each managed quest's exact ID, including explicit IDs. Invalid target syntax, absent aliases, duplicate quest slugs, serialized coordinate duplicates, divergent repeated groups, global collisions, and declared legacy reuse fail with catalog paths before quest-root mutation.
