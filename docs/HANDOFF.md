...
This is the current recovery record for Codex, Claude, or another capable agent. Last updated 2026-08-13 by Codex after the expedited RC2 production deployment.
...
- NeoForge: `21.1.248`.
- Java: `21`.
- Accepted RC2 commit, `dev`, `main`, annotated tag target, public Pages source, and live VPS marker: `c5993d1581bc7359eca503b99793da47bf4b9616`.
...
- RC2 public release: `https://github.com/Luskish/afterlight-pack/releases/tag/v1.0.0-rc.2`, published `2026-08-13T23:50:25Z`.
- Accepted RC2 receipt SHA-256: `7f1e37ecea320beb728483547bab780b33c1d7f42dda49ee22715ee3da49c687`.
- Accepted RC2 transcript SHA-256: `a76c25e352e188b85a761ab2c0a6e852d039f216002daf9926fa2ff9b2526ef8`.
- Exact RC2 `dev` CI: `https://github.com/Luskish/afterlight-pack/actions/runs/31747999995`.
- Exact RC2 `main` CI: `https://github.com/Luskish/afterlight-pack/actions/runs/31755216256`.
- The release portal selects RC2, and the live VPS now runs the exact RC2 commit with byte-identical protected player progress.
- Historical RC1 repair remains available if needed: an operator can repair only Slow Fire with `/ftbquests change_progress @a complete 78E07ADD5BCD1F19` and only Steel Yourself with `/ftbquests change_progress @a complete 27F6D0AB957BBB8C`. Both commands affect online players selected by `@a`; replace `@a` with one username for an individual repair.

### Story cohesion checkpoint

- Accepted RC2 contains the compatibility baseline, first-class quest-link compiler, race-safe transactional legacy overlays, deterministic generated-corpus snapshots, five approved common commodity declarations, eight optional field manuals, cohesive Story links and prose, exact effective-runtime acquisition declarations for all 81 manual quests, and the integrated quest-safe deployment control plane.
- Existing quest, chapter, task, reward, and group IDs remain frozen. RC2 is committed, published, promoted to `main`, selected by the portal, and deployed to production.
...
- The portal is live, selects the complete public `v1.0.0-rc.2` release without fallback, and renders all three launcher downloads.
...
- Repository: `/opt/afterlight`, with a root-owned control plane and runtime data owned by the non-login `afterlight` identity.
- RC1 rollback bridge: `42d924f9d393c0973cb217d188e290cd29ac6428` on retained branch `codex/rc1-control-bridge`.
- Current checkout, `origin/main`, `origin/dev`, and live marker: `c5993d1581bc7359eca503b99793da47bf4b9616`.
...
### RC2 deployment evidence

- Zero players were online before maintenance.
- Fresh recoverable backup: `/srv/afterlight/backups/world-20260814-011813.tar.zst`, SHA-256 `68160c0a9d5afcdd83acf05cb81464cc0b3290ed00901a61b06825ff613ff516`.
- Offline progress archive: `/srv/afterlight/backups/protected-progress-pre-rc2-20260814T011904Z.tar.zst`, SHA-256 `0b2a5334e6c80d3abef1b6ccf5b044ffe271a484873c9bbdf06959de990745de`.
- All 12 protected FTB Quests and FTB Teams progress records retained the exact pre-update manifest SHA-256 `657af03b67f59a4197101eb78505e8f6002b2e9ca5b666d42109d18139c539ba` after RC2 reached healthy state.
- The live quest corpus is FTB's runtime serialization rather than byte-identical source formatting. The repository's semantic verifier accepted 3,847 records with SHA-256 `9c9509ec8b34f0051555ec489f286842f85412563e32a2b5fee4c1b89fd4a2e6`.
- All 159 installed server mods matched `tools/server-mod-manifest-lock.json` by inventory, size, and digest.
- Minecraft reached `Done (3.886s)!`; Minecraft and backup containers are healthy, TCP `25565` listens, the maintenance timer is active, and no transaction authority or update firewall gate remains.
- A released client successfully joined RC2 after deployment. Do not repeat the offline progress-byte comparison while players are active because normal quest and team activity legitimately changes those files.
- The host uses Python `3.12.13` at `/opt/afterlight-python/3.12.13` for the hardened control plane. The runtime RCON secret is mode `0600`, owned by UID and GID `1000`, beneath root-owned mode `0700` `/etc/afterlight/secrets` so the unprivileged containers can read the bind-mounted file without exposing the directory on the host.
- Three control-plane compatibility defects remain documented follow-ups: the progress guard rejects FTB's newline-separated saved SNBT, the firewall cleanup parser rejects Ubuntu's explicit default `--reject-with icmp-port-unreachable`, and exact quest-byte verification does not account for FTB's characterized save normalization or the source-only `.afterlight-managed.json` file. All failed closed, and none is active on production.
...
```text
Open /private/tmp/afterlight-release-dev-8ec43b76969ada8dd5d1f0986777d886db0fa1cd and read its AGENTS.md, docs/PROJECT_MEMORY.md, and docs/HANDOFF.md. Invoke every applicable skill before acting. RC2 is published and live at exact SHA c5993d1581bc7359eca503b99793da47bf4b9616. Never use em dashes. Do not touch the dirty checkout at /Users/shaneliszewski/MinecraftTest. Start with the documented control-plane compatibility follow-ups or any newly reported critical gameplay bug. Do not regenerate quests or run Packwiz unless a pack-content fix requires it. Preserve quest IDs and live progress, use zero-player maintenance plus a verified backup for every VPS update, and report failures verbatim.
```
