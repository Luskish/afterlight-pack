...
- The fresh final server install reported `BOOT ORACLE: OK errors=14 warnings=473 named-residuals=39`, then `Ran 931 tests in 887.544s`, `OK`, and `SERVER BOOT: OK`. RC2 was committed at `3c0178463ef84a10b86acb090490b9a4b022e6d7`. Its first exact-head gauntlet correctly stopped before push because one live commodity-evidence test lacked the standard clean-worktree skip contract. Commit `f11eb476e3b00b8d9095b8aa89113da608c8c7df` fixed that issue, and its gauntlet passed the clean 931-test suite, Packwiz verification, dedicated-server boot, and the authenticated 931-test suite. It then stopped at ShellCheck because the controller lacked `SCRIPTDIR` resolution for sibling safety-contract includes. Commit `1a8b91190990fe136a6d3c4c1b213cb3094b172c` fixed that controller issue. A complete artifact preflight then built and inspected both launcher archives, and exposed the last stale lock: FTB Filter System raised the correct client count from 156 to 157. The corrected two-pass client install now prints `CLIENT INSTALL: OK`. The first incomplete gate is committing that count correction, followed by exact-head gauntlet acceptance, CI, publication, zero players, and a verified backup before using the quest-safe update.
