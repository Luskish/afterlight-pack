...
- **Summary:** Legacy overlay and managed quest builds now share one exclusive repository lock, render from an immutable same-device candidate, and promote through verified platform exchange compare-and-swap with ownership-aware rollback and retained recovery evidence on unresolved failure.
- **Evidence:** The initial focused TDD run ended with `Ran 0 tests in 0.007s` and `FAILED (errors=1)` because `afterlight_quests.legacy_quest_overlays` did not exist. The pre-review focused suite ended with `Ran 12 tests in 3.068s` and `OK`, but independent review then reproduced data loss and stale success with `COMMIT_RACE_CLOBBERED True`, `ROLLBACK_CLOBBERED_THIRD_PARTY True`, `MANAGED_STATE_OMITS_VICTIM True`, `PARTIAL_BUILD_DELETED_VICTIM True`, `VALIDATION_FAILURE_PARTIAL_OUTPUTS 11`, `SYMLINK_ACCEPTED_AND_REPLACED True`, `HARDLINK_NOOP_VIOLATION True True`, `CLEANUP_FAILURE_REPORTED_SUCCESS True`, and `NON_TARGET_RACE_STALE_AUDIT True`. The fix-round transaction and overlay suite then ended with `Ran 37 tests in 9.826s` and `OK`, covering races after final recheck, lost rollback ownership for existing and new files, failed pre-mutation exchange, symlink and hardlink rejection, device mismatch, cleanup and rollback failure, dependency races, poisoned state, isolated-stage failures, Git-object digest provenance, nested lookalikes, multiline localization, and byte idempotence. The relevant compiler and compatibility suite ended with `Ran 144 tests in 7.044s` and `OK (skipped=2)`; both skips require an authenticated live install and are outside this task. The project-memory suite ended with `Ran 7 tests in 0.006s` and `OK`; compile, diff check, generated-corpus diff, and U+2014 scan printed `OK` or `CLEAN`.
...
- **Impact:** Direct writers and whole-build orchestration reject reentrant or mismatched transactions, unsafe path topology, mutable ownership claims, and frozen-input drift. Candidate-stage or post-promotion validation failure leaves the real repository unchanged when transaction ownership is retained. If ownership or cleanup is lost, the writer reports unresolved and retained recovery paths instead of overwriting third-party bytes or claiming success.
