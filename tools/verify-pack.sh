...
for s in tools/export.sh tools/build-prism-instance.sh tools/build-release.sh tools/release-gauntlet.sh tools/server-test.sh server/afterlight-server.sh; do
  if bash -n "$s"; then
    echo "OK: $s parses"
  else
    echo "FAIL: $s syntax"
